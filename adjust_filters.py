...
def adjust_filters():
    ...
    if count < 10:
        logger.info("Недостаточно отзывов (нужно >= 10). Пропускаем.")
        return

    # Корректировка порога семантической схожести
    ...
    if avg_rating < 3.0:
        ...
        logger.info(f"Порог остался {current:.2f} (средняя оценка в норме)")

    # Анализ слов из отзывов
